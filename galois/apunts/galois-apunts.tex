...
\Cref{theo:normal and finite iff splitting field} is helpful in discussing the normality
...
\begin{proof}
	...
	must be normal by \Cref{theo:normal and finite iff splitting field}.
\end{proof}
...
\begin{definition}[Field fixed by a group]
...
\begin{proposition}
	...
	\begin{points}
	\item \locallabel{i} if \( L \in \L_K(F) \) then \( L \subseteq \Fi(\G(L)) =
		F^{\Gal_L(F)} \)
	\item \locallabel{ii} if \( H \in \L(\Gal_K(F)) \) then \( H \leq \G(\Fi(H)) =
		\Gal_{F^H}(F) \)
	\item \locallabel{iii} both \( \Fi \) and \( \G \) are decreasing, meaning if \( H_1 \leq
		H_2 \) then \( \Fi(H_1) \supseteq \Fi(H_2) \) and if \( L_1 \subseteq L_2 \) then \(
		\G(L_1) \geq \G(L_2) \)
	\item \locallabel{iv} \( \Fi \circ \G \circ \Fi = \Fi \) and \( \G \circ \Fi \circ \G =
		\G \).
	\end{points}
\end{proposition}
\begin{proof}
	\localref{i} Let \( x \in L \). Then, for any \( \sigma \in \Gal_L(F) \) one has, by
	definition, \( \sigma(x) = x \). Thus \( x \in F^{\Gal_L(F)} \), which means \( L
	\subseteq F^{\Gal_L(F)} \).

	\localref{ii} Let \( h \in H \). Then, by definition, \( h \) is a field automorphism of
	\( F \) which fixes \( F^H \). Thus \( h \in \Gal_{F^H}(F) \) and \( H \subseteq
	\Gal_{F^H}(F) \).

	\localref{iii} Let \( H_1 \leq H_2 \leq \Gal_K(F) \). Then, any element of \( \Fi(H_2) =
	...
	\localref{iv} Let \( H \leq \Gal_K(F) \). Then we need to show
	\begin{equation*}
		(\Fi \circ \G \circ \Fi)(H) = F^{\Gal_{F^H}(F)} = F^H.
	\end{equation*}
	Using \localref{i} we have
	\begin{equation*}
		(\Fi \circ \G \circ \Fi)(H) = (\Fi \circ \G)(\Fi(H)) \supseteq \Fi(H).
	\end{equation*}
	The other inclusion remains to be shown. We need to see that if \( x \in
	F^{\Gal_{F^H}(F)} \) then \( x \in F^H \). From \localref{ii} follows that \(
	\Gal_{F^H}(F) = (\G \circ \Fi)(H) \geq H \). Therefore, from \localref{iii} we
	conclude 
	\begin{equation*}
