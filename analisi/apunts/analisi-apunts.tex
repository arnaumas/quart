...
\declaretheorem[name=Lemma, refname={lemma,lemmata}, Refname={Lemma,Lemmata}, numberlike=theo]{lemma}
...

{
	...
	\begin{prop}[Properties of the exterior measure] \label{prop:properties of exterior measure}
...
\begin{prop}
	The set of measurable sets is a \( \sigma \)-algebra.
\end{prop}
\begin{proof}
	Let \( \M \) denote the set of measurable subsets of \( \R^n \). By the previous example we have that \( \R^n, \emptyset \in \M \). We need to show that it is closed under unions and complements. It is certainly closed under complements since \( (E^c)^c = E \). 
	
	We will first show that it is closed under finite unions. For any \( A \subseteq \R^n \) and \( E, F \in \M \) measurable sets we have
	\begin{equation*}
	...
	\end{align*}
	where we have used the measurability of \( E \) and \( F \) in the last two steps. Arguing by induction we find that \( \M \) is closed under finite unions.
\end{proof}
